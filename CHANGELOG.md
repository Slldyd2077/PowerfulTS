...
- **music**: 机器人行为面板新增「语音闪避」全局设置（开关 + 说话时音乐音量 0-100%）：频道里有人说话时音乐自动压低、说完渐升恢复，不打断播放；原生客户端与网页通话用户的说话均可触发（需搭配 TSMusicBot 专用分支 v1.12.1+）。

### Fix

- **social**: 好友申请支持在「好友」页面直接接受或拒绝（此前后端接口就绪但无前端入口），申请通知文案同步由「管理后台」修正为「好友」页面。

## v0.12.5 (2026-08-12)

### Fix

- **ui**: 浏览器标签标题简化为 PowerfulTS；侧栏游客通话与频道锁提示改用固定 rem 字号并加粗，缩放下不再过小难辨。
...
- **voice**: 游客模式改为由后端签发两小时临时身份，可使用 TeamSpeak 网页通话；音乐、好友、Steam 与管理接口继续保持隔离。

### Fix

- **voice**: 增加游客身份申请、麦克风操作与全局席位限制，并在挂断、断开、放弃加入或会话过期后回收临时 TeamSpeak Bot。

## v0.12.2 (2026-08-07)

### Fix

- **voice**: 网页通话身份的 `<WEB通讯>` 昵称前缀不再影响身份匹配，从网页加入通话也会触发好友 QQ 上线提醒（好友在线状态、Steam 当前游戏同样恢复）。
- **voice**: 加入通话 / 切频道 / 挂断后强制同步一次 TS 快照，页面立即反映新状态，不再等下一个轮询周期，也不会被旧快照弹回原频道。
- **voice**: 通话机器人不再被登记为「首次进入服务器的新成员」，避免误报新人提醒。
...
- 新增经用户主动授权的应用/系统音频实时共享，可将浏览器捕获的音频通过音乐机器人播放到 TeamSpeak 频道。
- 为实时音频链路增加一次性会话、WebSocket 上传与单消费者流式转发。
